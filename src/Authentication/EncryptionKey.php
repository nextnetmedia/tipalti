   ...
   */
    public static function generate($className, array $args, $apiKey)
    {
        if (!isset($args['payerName'])) {
            throw new Exception("You must specify a payerName");
        }
        if (!isset($args['timestamp'])) {
            throw new Exception("You must specify a timestamp");
        }
        ...
        if ($nestedAnnotation && !empty($nestedAnnotation->childItem) && isset($args[$nestedAnnotation->childItem]) && is_object($args[$nestedAnnotation->childItem])) {
            ...
            if ($annotation && ($annotation->eatParameter)) {
                foreach ($annotation->eatParameter as $av) {
                    if (method_exists($args[$nestedAnnotation->childItem], "get" . $av)) {
                        $value = $args[$nestedAnnotation->childItem]->{"get" . $av}();
                    } else {
                        $value = empty($args[$nestedAnnotation->childItem]->{$av}) ? null : $args[$nestedAnnotation->childItem]->{$av};
                    }
                    if (!empty($value)) {
                        break;
                    }
                ...
                if (method_exists($args[$nestedAnnotation->childItem], "get" . $annotation->eatParameter)) {
                    $value = $args[$nestedAnnotation->childItem]->{"get" . $annotation->eatParameter}();
                } else {
                    $value = empty($args[$nestedAnnotation->childItem]->{$annotation->eatParameter}) ? null : $args[$nestedAnnotation->childItem]->{$annotation->eatParameter};
                }
            }
        } else {
            ...
            if (is_array($annotation->eatParameter)) {
                foreach ($annotation->eatParameter as $av) {
                    if (strtolower($av) == "idap") {
                        if (!empty($args[ucfirst($av)])) {
                            return self::generateHmac([$args['payerName'], $args[ucfirst($av)], $args['timestamp']], $apiKey);
                        } elseif (!empty($args[lcfirst($av)])) {
                            return self::generateHmac([$args['payerName'], $args[lcfirst($av)], $args['timestamp']], $apiKey);
                        } elseif (!empty($args[strtolower($av)])) {
                            return self::generateHmac([$args['payerName'], $args[strtolower($av)], $args['timestamp']], $apiKey);
                        } else {
                            return self::generateHmac([$args['payerName'], $value, $args['timestamp']], $apiKey);
                        }
                    } else {
                        if (!empty($args[ucfirst($av)])) {
                            return self::generateHmac([$args['payerName'], $args['timestamp'], $args[ucfirst($av)]], $apiKey);
                        } elseif (!empty($args[lcfirst($av)])) {
                            return self::generateHmac([$args['payerName'], $args['timestamp'], $args[lcfirst($av)]], $apiKey);
                        } elseif (!empty($args[strtolower($av)])) {
                            return self::generateHmac([$args['payerName'], $args['timestamp'], $args[strtolower($av)]], $apiKey);
                        } else {
