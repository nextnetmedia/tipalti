        ...
        if (class_exists($class)) {
            ...
            foreach ($reflection->getParameters() as $param) {
                ...
                if (strtolower($paramName) == 'payername') {
                    ...
                    if (!isset($args[$index])) {
                        throw new Exception('Must pass ' . $paramName . ' as argument ' . ($index+1) . ' to ' . $name);
                    }
            ...
            if (method_exists($finalreply, "getErrorMessage") && strtolower(trim($finalreply->getErrorMessage(), " .")) !== 'ok') {
                if ($finalreply->getErrorCode() != $finalreply->getErrorMessage()) {
                    throw new Exception("Error " . $finalreply->getErrorCode() . ": " . $finalreply->getErrorMessage());
                } else {
                    throw new Exception("Error: " . $finalreply->getErrorCode());
                }
            } else {
                ...
                $replyArray = json_decode(json_encode($reply), true);
                $possiblyBrokenResponse = array_shift($replyArray);
